...
def run_dmft(U, T, pumpA, probeA, mu, v_0, tmax, dt, dw, tol, solver, phonon, fermion, Lambda, wC, t_diss_end, pumpOmega, t_pump_start, t_pump_end, probeOmega, t_probe_start, t_probe_end, lattice_structure, output, **kwargs):
    ...
    while diff > tol:
        ...
        plt.matshow(np.real(Green[0,0]), cmap=plt.get_cmap('gray'))
        plt.savefig('G_gtr_matrix_real.pdf')
        plt.colorbar()
        plt.close()

        plt.matshow(np.imag(Green[0,0]))
        plt.savefig('G_gtr_matrix_imag.pdf')
        plt.close()

        plt.matshow(np.real(Green[1,0]))
        plt.savefig('G_les_matrix_real.pdf')
        plt.close()

        plt.matshow(np.imag(Green[1,0]))
        plt.savefig('G_les_matrix_imag.pdf')
        plt.close()
