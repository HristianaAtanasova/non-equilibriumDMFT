...
def flatBand(w, wC, v, gamma):
    ...
    """
    return gamma  * w / ((np.exp(v * (w - wC)) + 1) * (np.exp(-v * (w + wC)) + 1))
...
'''
cwd = os.getcwd()
Path = os.getcwd() + '/U10.0_dt{}_pumpA{}_pumpOmega10.0/wC{}'

tmax = 10
Temp = [1.0]
wC_ = [10, 20, 30]
dt_ = [0.1, 0.05, 0.02, 0.01, 0.005]

for T in Temp:
    for wC in wC_:
        for dt in dt_:
            path = Path.format(dt, 0.0, wC)
            ...
            Delta_name = 'ferm_Delta_wC={}_E_T={}_dt={}'
            np.savez_compressed(Delta_name.format(wC,T,dt), Delta=Delta)
