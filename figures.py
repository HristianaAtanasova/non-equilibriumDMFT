...
plt.rc('font',**{'family':'serif','serif':['Times'],'size': 7.0})
# plt.rc('font',**{'family':'serif','serif':['Times'],'size': 8.0})
...
def set_default_spacing(fig):
    ...
    fig.subplots_adjust(top=1.0 - 0.05 * fig.default_height / fig.get_size_inches()[1])
...
def finalize_and_save(fig, filename='plot.pdf'):
    axes = fig.get_axes()
    for ax in axes:
        # legend = ax.legend(loc='upper right', fontsize=5.0, fancybox=True, framealpha=0.5)
        # legend = ax.legend(loc='lower left', fontsize=5.0, fancybox=True, framealpha=0.5)
        legend = ax.legend(loc=(0.1, 0.1), fontsize=5.0, fancybox=True, framealpha=0.5)
